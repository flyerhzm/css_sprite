...
In practice, we may use the same class under different parent classes to display different images. Such as <code>.large_widget .content_bg</code> and <code>.small_widget .content_bg</code>. css_sprite gem uses directory to represent the parent class.
...
Hover selector is another frequently-used css technique. css_sprite gem uses the "_hover" and "-hover" suffix to detect it.
